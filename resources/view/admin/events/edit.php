...
<?= renderComponent(CrumbsComponent::class, ['name' => 'Update Event']); ?>
    ...
    <div class="row g-5 mb-3">
        ...
        <div class="col-md-7 col-lg-8">
            <form action="" method="post" enctype="multipart/form-data">
                <div class="row g-3">
                    <div class="d-flex align-items-center">
                        ...
                        <?php if ($upload_type === "file") : ?>
                            <a href="<?= URL_ROOT . "/admin/events/edit/{$event['event_id']}?ut=link" ?>" class="text-black my-2"> <i class="fa-solid fa-link"></i> Link Upload</a>
                        <?php else : ?>
                            <a href="<?= URL_ROOT . "/admin/events/edit/{$event['event_id']}?ut=file" ?>" class="text-warning my-2"> <i class="fa-solid fa-file-import"></i> File Upload</a>
                        <?php endif; ?>
                    </div>
                    ...
                    <?= BootstrapForm::inputField("Social Twitter (X)", "socials[]", old_value("socials[]", $links["twitter"] ?? ''), ['class' => 'form-control', 'type' => 'url'], ['class' => 'col-sm-6'], $errors) ?>

                    <?= BootstrapForm::inputField("Social Facebook", "socials[]", old_value("socials[]", $links["facebook"] ?? ''), ['class' => 'form-control', 'type' => 'url'], ['class' => 'col-sm-6'], $errors) ?>

                    <?= BootstrapForm::inputField("Social Instagram", "socials[]", old_value("socials[]", $links["instagram"] ?? ''), ['class' => 'form-control', 'type' => 'url'], ['class' => 'col-sm-6'], $errors) ?>

                    <?= BootstrapForm::inputField("Social Tik Tok", "socials[]", old_value("socials[]", $links["tiktok"] ?? ''), ['class' => 'form-control', 'type' => 'url'], ['class' => 'col-sm-6'], $errors) ?>
                ...
                <div class="row gap-3">
                    <a href="<?= URL_ROOT . "/admin/events/manage" ?>" class="btn btn-danger btn-lg col-4">Cancel</a>
                    <button class="btn btn-dark btn-lg col" type="submit">Update Event</button>
                </div>
            </form>
        </div>
    </div>
